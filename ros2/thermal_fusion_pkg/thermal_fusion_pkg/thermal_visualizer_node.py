...
class ThermalVisualizer(Node):
    ...
    def __init__(self):
        ...
        self.create_subscription(Float32MultiArray, '/machine_temperatures', self.on_temperatures, 10)
        self.create_subscription(Float32MultiArray, '/machine_world_positions', self.on_positions, 10)
        ...
        self.raw_pub = self.create_publisher(String, '/thermal/raw_values', 10)
        
        self.create_timer(0.1, self.render)
        self._gauss_cache: dict = {}

        self.get_logger().info('ThermalVisualizer v5.5 Ready [Overheating Bug Fixed]')
    ...
    def render(self):
        ...
        measured_temperatures = [0.0] * len(self.machine_boxes)
        visible_machines = {}
        ...
        for i, ((cx, cy, cz), (ex, ey, ez)) in enumerate(self.machine_boxes):
            ...
            norm_val = float(np.clip((self.temperatures[i] - TEMP_MIN) / (TEMP_MAX - TEMP_MIN), 0, 1))
            ...
            for sx in (-1, 1):
                for sy in (-1, 1):
                    for sz in (-1, 1):
                        points_map.append((cx + sx * sex, cy + sy * sey, cz + sz * sez))
            ...
            if center_uv is None or len(projected_pts) < 2:
                continue

            margin_x = self.img_w * OFFSCREEN_MARGIN_RATIO
            margin_y = self.img_h * OFFSCREEN_MARGIN_RATIO
            if (center_uv[0] < -margin_x or center_uv[0] > self.img_w + margin_x or
                center_uv[1] < -margin_y or center_uv[1] > self.img_h + margin_y):
                continue

            # 화각(FOV) 바운더리 검사를 통과한 장비의 순수 원본값 매핑 (오차 완벽 배제)
            if (0 <= center_uv[0] < self.img_w and 0 <= center_uv[1] < self.img_h):
                estimated_temp = TEMP_MIN + norm_val * (TEMP_MAX - TEMP_MIN)
                measured_temperatures[i] = estimated_temp
                
                machine_name = MACHINE_IDS[i] if i < len(MACHINE_IDS) else f'machine_{i+1}'
                visible_machines[machine_name] = estimated_temp

            # 영상 드로잉 연산 수행
            pts_arr = np.array(projected_pts)
            ...
            self._add_blob(heat_accum, center_proj, sigma=base_sigma * 1.1, weight=CENTER_WEIGHT * norm_val)
            corner_sigma = base_sigma * 0.7
            for cp in corner_projs:
                self._add_blob(heat_accum, cp, sigma=corner_sigma, weight=CORNER_WEIGHT * norm_val)

        # 알람 노드로 화각 내 선별된 JSON 데이터 전송
        raw_string_msg = String()
        raw_string_msg.data = json.dumps(visible_machines)
        self.raw_pub.publish(raw_string_msg)

        if FINAL_BLUR_K > 1:
            heat_accum = cv2.GaussianBlur(heat_accum, (FINAL_BLUR_K, FINAL_BLUR_K), 0)
        ...
        for i, pix in enumerate(pixel_list):
            ...
            t_measured = measured_temperatures[i]
            name = MACHINE_IDS[i] if i < len(MACHINE_IDS) else f'M{i+1}'
            
            cv2.circle(colormap, (u, v), 5, self._status_color(t_measured), -1)
            cv2.putText(colormap, f'{name}: {t_measured:.1f}C', (u + 7, v - 4),
